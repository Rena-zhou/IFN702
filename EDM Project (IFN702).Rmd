...
#parameterGrid <- expand.grid(mtry=c(2,3,4,5,6,7,8,9))

modelRandom <- train(Class~.,
                     data = E,
                     method = "rpart",
...
rf.model <- randomForest(Class ~ . , data = train_data, importance = TRUE,
...
TrainingDataIndex <- createDataPartition(E$Class, p=0.80, list = FALSE)

# Create Training Data 
trainingData <- E[TrainingDataIndex,]
testData <- E[-TrainingDataIndex,]
...
# Create confusion matrix
rfNN <-confusionMatrix(rfPredictions, testData$Class)
...
# Create confusion matrix
SVMNN <-confusionMatrix(SVMPredictions, testData$Class)
