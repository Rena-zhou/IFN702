...
Ed$Class = factor(Ed$Class, levels=c("H","M","L"), labels=c("H","M","L")) 

Ed$StageID = factor(Ed$StageID, levels=c("HighSchool","MiddleSchool","lowerlever"), labels=c("HighSchool","MiddleSchool","lowerlever")) 


ggplot(data = Ed, aes(x = StageID, fill = Class)) + geom_bar() +
    labs(x = "StageID", y = "Student Count") +theme(axis.text.x = element_text(angle=90, hjust=1))
```
...
#Ed = ordered(Ed$Class, 
            levels = c("H","M","L","M.G","M.R","M.B","H.G","H.R","H.B")
...
#levels(E$Class) = c('H','M','L')
ggplot(data = Ed, aes(x = Topic, fill = Class)) + geom_bar() +
    labs(x = "Topic", y = "Student Count") + ggtitle(label = "Topic Distribution")+theme_grey()+theme(plot.title = element_text(hjust = 0.5, lineheight = 0.8, face = "bold"))+xlab("Topic")+theme(axis.text.x = element_text(angle=90, hjust=1))
