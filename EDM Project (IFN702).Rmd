...
#Task 1.2. Barplots of classification values distribution:
...
# Summary of Task 1.1: Based on the exploration work above, We conclude the following main findings which will be much useful for analysis and prediction of the academic performance in the next steps:
...
# Summary of Task 1.2: From the exploration of the numerical feature values, we have the following findings:
1 .Students who have the higher class raised hands, visited resource, viewd announcements and discussed more times.

2. Most of the students with high class raised hands, visited resource, viewd announcements and discussed many times, most of the students with low class raised hands, visited resource, viewd announcements and discussed few times. 

3. There are some little differences in the density curve, especially in the announcement view and disussion.
...
1 . From the relationship between class and other features, it further confirmed our findings in the previous step: Students who have the higher class raised hands, visited resource, viewd announcements and discussed more times.

2. Girls have more hand raises and visit more resources. 

3. The results on raised hands, visited resources and announcements view are pretty similar.The differences of discussions between Jordan and Kuwait are not obvious.

4. More raised hands, visited resources and announcements view in middle schools. More discussions in high schools.

5. IT has very few hand raises intersetingly although most students study there.
...
#Summary of model 1: we can get some findings from the tree mode above:
1. From the plot of the tree, some variables ( VisITedResources, StudentAbsenceDays, Relation, gender, ParentAnsweringSurvey and raisedhands) are selected as split condtions, which means these featureas are crucial in spliting the tree. so they are very important features in this tree model.

2. From the confusion matrix and heatmap, there is no prediction error between class L and H. However, the performance is not so good between class M and H, M and L. It means that this model have some problem in predicting the adjacent classes but not the distant classes.

3. The accuracy of this model is 0.7038 from the statistics result. The accuracy is a description of systematic errors, a measure of statistical bias.It relects percetage of the true positive cases out of all the cases. In this case, we can see that 24 of H class have been predicted correctly which means the numbers of the true possitive of H class, and 7 of M classes have been predicted as H classes but acctually not H, which indicates numbers of the false possitive of H class. To calculate accuracy, the true positive cases are accumulated as 84 (24+40+21) including all the H,M and L classes, and the total cases are 120 (24+7+11+40+13+4+21) including all the TP, TN, FP and FN cases. The formula of accuracy is TP/TP+TN+FP+FT,so the accuracy is 84/120 = 0.7038 in this model.The accuracy reflects how the model performe in predicting the correct classes out of all the cases.

4. Precision is a better method for us to judge the performance of a model. Precision is a description of random errors, a measure of statistical variability. The formula of precision is TP/TP+FP, which can reflect the rate of true positive cases out of all the positve cases including false positive cases. In this case, there are 13 L classes that predicted as M classes, these 13 cases is the FP cases in predicting M classes, and there are also 7 M classes predicted as H classes, these 7 cases is the FP cases in predicting H classes. We can get a better insight of the model by refering to the precision of each class.

5. Since the precision is related to the true possitive and false posive, we can evaluate the performance of the predictive model by calculating the precision of each class. In this confusion matrix, the specifity is equal to precision, we can use the specifity of the results of confusion matrix to observe the performance of the model.

In this tree model, specifity is: H(0.9176), M(0.6522), L(0.9535), and the mean specifity is 0.8411.


#Prediction model 2 -- Randomforest
...
#Summary of model 2: we can get some findings from the tree mode above:
...
2. From the confusion matrix, there is also no prediction error between class L and H in this random forest model. There are some improvements: all the number of correct prediciton has been increased in all the classes (26, 42 and 26) than the previous model. And the FP of M classes have been reduced a lot. However the FP cases of H and L classes have been slightly increased.

3. The accuracy of this model 2 is 0.7583, which is a little higher than model 1.

4. In this random forest model, precision is: H(0.8602), M(0.8448), L(0.9213), and the mean precision is 0.8754.
...
```
#Summary of model 3: we can get some findings from the tree mode above:
1. From the confusion matrix, differenct from the previous two models, there is a prediction error between class L and H, one H class is predicted as L class. There are some improvements: all the number of correct prediciton has been increased in all the classes (29, 41 and 25), much better than the model 1, but similar to model 2. And the FP of H and M classes have been reduced, which is the best among these 3 models. 

3. The accuracy of this model 3 is 0.7917, which is a little higher than model 1 and model 2.

4. In this random forest model, precision is: H(0.9277), M(0.8462), L(0.9022), and the mean precision is 0.8920.
...
2. From the confusion matrix, there is a prediction error between class L and H, one H class is predicted as L class. There are some improvements: The numder of TP of H class with 30 is much better than the model 1 without cross validation, it's also the highest till now. And the FP are much samller than the model 1 without cross validation. However, there are 12 m class predicted as L class, whichi is much high than the model 1 without cross validation.
...
Overall, this model is better than the model 1 without cross validation both in accuracy and precision, and similiar to the perfomance of model 2 without cross validation
...
2. From the confusion matrix, there is no prediction error between class L and H in this random forest model. There are some improvements: The number of correct prediciton has been increased a lot in M and L class (43 and 28) than the model 1. And there are only 4 m class predicted as L class, whichi has been improved a lot than model 1.
...
2. From the confusion matrix, there is no prediction error between class L and H in this random forest model. There are some improvements than model 1 but not than model 2: The number of correct prediciton has been increased a lot in M and L class (43 and 25) than the model 1. And there are only 4 m class predicted as L class, whichi has been improved a lot than model 1. However, the number of TP of H class is the lowest among those 3 models, even worse thant the model 3 without cross validation.
...
1. Model 1 and 2 performed much better by training on the N-folder cross validation dataset, but model 3 performed even worse.

2. Model 2 (Random forest) outperformed all the models by training on the N-folder cross validation dataset. It got the highest rate of accuracy and precison.

3. The most important activities that related to students' academic performance are VisITedResources, raisedhands, StudentAbsenceDays, AnnouncementsView and Discussion. Then the Relation, ParentAnsweringSurvey, gender and ParentschoolSatisfaction also has some connection with the academic performance. Students' academic performance has little connection with topic, StageID, Nationality, PlaceofBirth, Grade ID, Section ID and semester.
