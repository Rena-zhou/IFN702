...
ind <- sample(2, nrow(E), replace=TRUE, prob=c(0.7, 0.3))
train_data <- E[ind==1,]
test_data <- E[ind==2,]
```
...
E1 <- table(predict(E_ctree, newdata = test_data), test_data$Class)
E1
```

```{r evaluate performance of decision tree 2}
confusionMatrix(predict(E_ctree, newdata = test_data), test_data$Class)
```
...
* Produce the heatmap for 70-30 split
```{r}
M1 <- melt(E1)
names(M1) <- c("Prediction", "Reference", "Value")
M1$Value <- (M1$Value - min(M1$Value))/(max(M1$Value) - min(M1$Value))
M1

ggplot(M1, aes(x=Reference, y=Prediction, fill=Value)) + geom_tile() + scale_fill_gradient(low = "white",high = "red") + scale_x_discrete(position = "top") + scale_y_discrete(limits = rev(levels(M1$Prediction)))
```
