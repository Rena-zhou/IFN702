...
#+ scale_fill_discrete(limits=c("H","M","L"))
...
ggplot(ESummary, aes(x = Topic, y = count)) + 
 ...
 geom_text(aes(y = pos, label = Percent))  + ggtitle(label = "Topic Distribution")+theme_grey()+theme(plot.title = element_text(hjust = 0.5, lineheight = 0.8, face = "bold"))+xlab("Topic")+ylab("Counts")+theme(axis.text.x = element_text(angle=90, hjust=1)) 
...
```{r}
Topic_frequence <- as.data.frame(table(E$Topic))
names(Topic_frequence)<- c("Topic","Counts")

Edu <- cbind(Topic_frequence, E)
#names(Topic_f)<- c("Topic","Counts","Class")
#Topic_f
Edu

#Topic_f$Counts = Topic_f$Counts/Topic_frequence$Counts

ggplot(Edu, mapping = aes(x = Topic, y = Counts, fill = Class)) + geom_bar(stat= 'identity') +
    labs(x = "Topic", y = "Student Count") + ggtitle(label = "Topic Distribution")+theme_grey()+theme(plot.title = element_text(hjust = 0.5, lineheight = 0.8, face = "bold"))+xlab("Topic")+ylab("Counts")+theme(axis.text.x = element_text(angle=90, hjust=1))
...
Topic_f <- cbind(Topic_frequence, E$Class)
names(Topic_f)<- c("Topic","Counts","Class")
Topic_f

ce <- ddply(.data = cabbage_exp, .variables = "Topic", .fun = transform, percent_weight = Weight/sum(Weight) * 
    100)
head(cabbage_exp)
...
```{r}
ggplot(data = E, aes(x = raisedhands, color = Class)) + geom_density()
```
